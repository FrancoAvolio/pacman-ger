...
describe('ghost AI', () => {
  ...
  it('releases ghosts at configurable staggered boundaries', () => {
    ...
    expect(isGhostReleased('ORANGE', 6_249, level, 'tranqui')).toBe(false)
    expect(isGhostReleased('ORANGE', 6_250, level, 'tranqui')).toBe(true)
    expect(isGhostReleased('ORANGE', 4_399, level, 'arcade')).toBe(false)
    expect(isGhostReleased('ORANGE', 4_400, level, 'arcade')).toBe(true)
